...
用法:
  ...
  python run.py --no-trt           # 关闭 TensorRT
  python run.py --help
"""
import argparse
import os
...
def main():
    ...
    parser.add_argument("--no-trt", action="store_true", help="关闭 TensorRT（默认开启）")
    parser.add_argument("--no-vllm", action="store_true", help="关闭 vLLM（默认开启）")
    parser.add_argument("--no-fp16", action="store_true", help="关闭 FP16（默认开启）")
